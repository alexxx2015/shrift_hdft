...
public class UcCommunicator {
	...
	private UcCommunicator() {
		...
		this.PIP_HOST = ConfigProperties
				.getProperty(ConfigProperties.PROPERTIES.PIP_HOST);
		this.PIP_PORT = ConfigProperties
				.getProperty(ConfigProperties.PROPERTIES.PIP_PORT);
		...
		this.async = false;
		boolean pdpAsync = new Boolean(
				ConfigProperties
						.getProperty(ConfigProperties.PROPERTIES.PDP_ASYNCOM));
		if (pdpAsync == true)
			this.async = true;

		this.netcom = false;
		boolean configNetcom = new Boolean(
				ConfigProperties
	...
	protected void initPDP() {
		...
		if (netcom) {
			try {
				if (this.pdpClient == null) {
					...
					this.pipClient = thriftClientFactory
							.createAny2PipClient(new IPLocation(this.PIP_HOST,
									Integer.parseInt(this.PIP_PORT)));
			...
			if (this.pdpController == null) {
				...
				String uc4win_autostart = ConfigProperties
						.getProperty(ConfigProperties.PROPERTIES.UC4WIN_AUTOSTART);
				if (!"".equals(uc4win_autostart)
						&& !"".equals(uc4win_autostart.trim())) {
					uc4win_autostart = uc4win_autostart.trim();
					String[] start = new String[] { "cmd.exe", "/c", "sc",
							"start", uc4win_autostart };
					try {
	...
	public boolean sendEvent2Pdp(IEvent event, String p_methName) {
		event.getParameters().put("ThreadId",
		...
		if (timer5) {
			...
			StatisticsWriter.logExecutionTimerT5(p_methName, System.nanoTime()
					- start);
	...
	public Map<String, Set<Map<String, String>>> filterPipDataModel(
	...
	public boolean sendKillProcessEvent2Pdp() {
		...
		if (!"".equals(uc4win_autostart) && !"".equals(uc4win_autostart.trim())) {
			uc4win_autostart = uc4win_autostart.trim();
			String[] start = new String[] { "cmd.exe", "/c", "sc", "stop",
					uc4win_autostart };
			try {
				Runtime.getRuntime().exec(start);
				Thread.sleep(1500);

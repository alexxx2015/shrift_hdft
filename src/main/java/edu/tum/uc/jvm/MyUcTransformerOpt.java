...
//		UcCommunicator.getInstance().initPDP();
	...
	public MyUcTransformerOpt() {
		this(false);
	}

	public MyUcTransformerOpt(boolean p_instrument_webservice) {
	...
	private void setClassLoader(ClassLoader p_myClassLoader) {
		MyUcTransformerOpt.myClassLoader = (TomcatClassLoader) p_myClassLoader;
	}

	public static ClassLoader getMyClassLoader() {
		return MyUcTransformerOpt.myClassLoader;
	}

	private void setProtectionDomain(ProtectionDomain p_protectionDomain) {
		MyUcTransformerOpt.myProtDom = p_protectionDomain;
	}
...
//				System.out.println("DUMPING class "+className);
		...
		// delegate
		InstrumDelegateOpt.addInstrumentedClassName(cr.getClassName());

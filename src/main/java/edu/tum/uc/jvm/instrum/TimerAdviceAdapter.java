...
public class TimerAdviceAdapter extends AdviceAdapter {
    ...
    private boolean shouldAddTimer() {
    	...
    	Map<String, List<Map<String, String>>> json = new Gson().fromJson(jsonString, new TypeToken<Map<String, List<Map<String, String>>>>(){}.getType());
    	...
    	for (Map<String, String> method : methods) {
    		...
    		for (Entry<String, String> keyValuePair : method.entrySet()) {
    			if (keyValuePair.getKey().equals("superClassName"))
    				...
    				match = match && Type.getArgumentTypes(this.descriptor).length == Integer.parseInt(keyValuePair.getValue());
    	...
    	return false;
//	return (this.superClassName.contains("Servlet") && Type.getArgumentTypes(this.descriptor).length == 2
